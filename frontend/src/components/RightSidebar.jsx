    ...
    <>
      <div className="relative">
        {props.infoPanelIsOpen && (
          <div onClick={() => props.setInfoPanelIsOpen(false)} className="fixed top-0 right-0 bottom-0 w-screen h-screen backdrop-blur-lg bg-black bg-opacity-50"></div>
        )}
        <div
          className={`absolute right-0 top-0 w-[450px] bg-[#242424] h-screen ease transition duration-500 overflow-hidden 
      ${
        props.infoPanelIsOpen ? `translate-x-0` : `translate-x-full`
          ...
          <div className="pt-8 pb-12 p-6 flex flex-col justify-center items-center border-b border-zinc-700 relative">
            ...
            {props.infoPanelIsOpen && (
              <div className="absolute top-0 right-0 px-6 py-9">
                <button
                  onClick={() => props.setInfoPanelIsOpen(false)}
